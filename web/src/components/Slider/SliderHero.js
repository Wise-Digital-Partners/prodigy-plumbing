...
const StyledSlider = styled.div`
  ...
  .slick-dots {
    ...
    li {
      ...
      &.slick-active {
        button {
          ${tw`bg-primary-500 w-3 h-3`}
        }
      }
    }
    button {
      ${tw`w-2 h-2 bg-primary-500 bg-opacity-20 rounded-full mb-3 transition ease-in duration-300`}
      &:before {
        ${tw`hidden`}
      }
    }
  ...
  .slick-next {
    ...
    i {
      ${tw`text-primary-500 text-sm transition-all duration-300 ease-linear`}
    }
    &:hover {
